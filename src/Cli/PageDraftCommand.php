...
/**
 * Sets a page's own `draft` flag to true — the page, and every page nested
 * beneath it regardless of their own flag, 404s publicly and is excluded
 * from listings (see Page::isHidden()). The inverse of PagePublishCommand.
 */
#[AsCommand(
    name: 'page:draft',
    description: 'Mark a page (and its descendants, by cascade) as a draft',
)]
final class PageDraftCommand extends Command
{
    use WritesCliError;
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!str_ends_with($entry, '.json')) {
            return $this->fail(
                ...
                $json,
                sprintf(
                    '"/%s" uses a %s entry; page:draft only edits +page.json.',
                    $route,
                    basename($entry),
                ),
            );
     ...
     * a *bare* empty array encodes as `[]`, and $meta is a bare array by the
     * time execute() unsets a page's only key.
     *
     ...
     * page can be a draft with its own state, same as any other page.
     *
     * @return list<string>|null
     */
    ...
    private function report(OutputInterface $output, bool $json, string $route, bool $changed): int
    {
        ...
        $output->writeln(
            $changed
                ? sprintf('%s is now a draft.', $path)
                : sprintf('%s is already a draft.', $path),
        );
